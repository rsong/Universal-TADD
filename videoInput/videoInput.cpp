...
int videoInput::start(int deviceID, videoDevice *VD){
	...
	hr = changeSettings(VD->pVideoInputFilter, VD->pCaptureGraph);
	if (FAILED(hr))
    {
		if(verbose)printf("Fuck");
		stopDevice(deviceID);
		return hr;
    }
...
HRESULT videoInput::changeSettings(IBaseFilter *CapFilter, ICaptureGraphBuilder2 *g_pCapture)
{
	 HRESULT hr; 
	 IAMVideoProcAmp *pAMP;
	 IAMCameraControl *pBZ;


	 hr = g_pCapture->FindInterface(&PIN_CATEGORY_CAPTURE, &MEDIATYPE_Video,CapFilter,IID_IAMVideoProcAmp, (void **)&pAMP);
	 hr = CapFilter->QueryInterface(IID_IAMCameraControl, (void **)&pBZ);

	 long val,flag;

	 //pBZ->Get(CameraControl_Exposure,&val,&flag);
	 pAMP->Get(VideoProcAmp_WhiteBalance,&val,&flag);


	 pAMP->Set(13,1,VideoProcAmp_Flags_Manual); //aint flicker off
	 pAMP->Set(VideoProcAmp_Brightness,128,VideoProcAmp_Flags_Manual);
	 pAMP->Set(VideoProcAmp_Contrast,32,VideoProcAmp_Flags_Manual);
	 pAMP->Set(VideoProcAmp_Saturation,32,VideoProcAmp_Flags_Manual);
	 pAMP->Set(VideoProcAmp_WhiteBalance,4900,VideoProcAmp_Flags_Manual);
	 pAMP->Set(VideoProcAmp_BacklightCompensation,1,VideoProcAmp_Flags_Manual);
	 pAMP->Set(VideoProcAmp_Gain,16,VideoProcAmp_Flags_Manual);

	 hr = pBZ->Set(CameraControl_Exposure, -2, CameraControl_Flags_Manual); 
	 //pAMP->Set(VideoProcAmp_Gain,15,VideoProcAmp_Flags_Manual);
